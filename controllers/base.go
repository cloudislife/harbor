...
// Jump handles login request from UI.
func (cc *CommonController) Jump() {
	userId := cc.GetString("userId")
	if len(userId) == 0{
		log.Errorf("Error occurred in Jump as userId is empty!")
	}
	query := models.User{Username:userId}
	user,err:=dao.GetUser(query)
	if err != nil{
		log.Errorf("Error occurred in Jump: %v", err)
	}
